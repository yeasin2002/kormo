	...
	return (
		<div className="relative overflow-hidden">
			<div>
				...
				<div className="absolute right-20 bottom-52 text-foreground opacity-40">
					<Sparkles className="h-8 w-8" />
				</div>
			...
			<div className="container mx-auto px-6">
				<div className="mx-auto flex max-w-4xl flex-col items-center justify-center text-center">
					<div className="relative mb-12">
						<div className="select-none font-black text-[12rem] text-foreground leading-none opacity-20 md:text-[16rem] lg:text-[20rem]">
							404
						</div>
						<div className="absolute inset-0 flex items-center justify-center">
							<div className="-rotate-3 transform rounded-2xl border-4 border-black bg-yellow-400 px-8 py-4 text-black shadow-xl transition-transform duration-300 hover:rotate-0">
								<span className="font-bold text-2xl md:text-3xl">Oops!</span>
							</div>
						</div>
					</div>

					<div className="mb-12 space-y-6">
						<h1 className="font-bold font-head text-4xl leading-tight md:text-5xl lg:text-6xl">
							Page Not <span className="text-yellow-500">Found!</span>
						</h1>
						<p className="mx-auto max-w-2xl font-sans text-lg text-muted-foreground leading-relaxed md:text-xl">
					...
					<div className="mb-20 flex flex-col justify-center gap-4 sm:flex-row">
						<Button
							onClick={() => router.back()}
							className={
								"flex items-center gap-x-2 rounded-lg border-2 border-black bg-yellow-400 px-8 py-4 font-semibold text-black text-lg shadow-md hover:bg-yellow-500"
							}
						>
						...
						<Link
							...
							className={cn(
								buttonVariants(),
								"flex items-center gap-x-2 rounded-lg border-2 border-foreground bg-background px-8 py-4 font-semibold text-foreground text-lg shadow-md hover:bg-muted",
							)}
						>
			...
			<div className="fixed bottom-6 left-6 rotate-3 transform rounded-lg border-2 border-black bg-red-400 px-4 py-2 text-black shadow-lg transition-transform duration-300 hover:rotate-0">
				<span className="font-bold text-sm">Error Code: LOST_IN_SPACE</span>
			</div>
		</div>
	);
}
