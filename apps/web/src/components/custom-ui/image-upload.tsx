		...
		<div className="flex items-center space-x-4">
			<div
				className={cn(
					...
					isDragging
						? "scale-105 border-yellow-400 bg-yellow-50/10"
						: "border-border",
			...
			>
				{isLoading ? (
					...
					<div className="flex h-full flex-col items-center justify-center p-2 text-center">
						<Upload className="mb-1 h-8 w-8 text-muted-foreground transition-colors group-hover:text-yellow-400" />
			...
			<div className="flex-1">
				...
				{error && (
					<p className="mt-1 text-red-500 text-sm">{error.message as string}</p>
				)}
			</div>
		</div>
	);
